...
class RandEdgeSampler(object):
    ...
    def sample(self, size):
        return np.random.randint(0, self.num_nodes, size=size)
...
class NeighborFinder:
    def __init__(self, adj_list, uniform=False, seed=None):
        ...
        for neighbors in adj_list:
            ...
            self.node_to_edge_timestamps.append(np.array(
                [x[2]
                 for x in
                 sorted_neighhbors]))
    ...
    def find_before(self, src_idx, cut_time):
        ...
        return self.node_to_neighbors[src_idx][
            : i], self.node_to_edge_idxs[src_idx][
            : i], self.node_to_edge_timestamps[src_idx][
            : i]
    ...
    def get_temporal_neighbor(self, source_nodes, timestamps, n_neighbors=20):
        ...
        for i, (source_node, timestamp) in enumerate(
                zip(source_nodes, timestamps)):
            ...
            if len(source_neighbors) > 0 and n_neighbors > 0:
                if self.uniform:  # if we are applying uniform sampling, shuffles the data above before sampling
                    ...
                    neighbors[i,
                              n_neighbors - len(source_neighbors):] = source_neighbors
                    edge_times[i,
                               n_neighbors - len(source_edge_times):] = source_edge_times
                    edge_idxs[i,
                              n_neighbors - len(source_edge_idxs):] = source_edge_idxs

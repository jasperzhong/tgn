...
class TGN(torch.nn.Module):
    def __init__(
            self, neighbor_finder, node_features, edge_features, device,
        ...
        if self.use_memory:
            ...
            self.memory_updater = get_memory_updater(
                module_type=memory_updater_type, memory=self.memory,
                message_dimension=message_dimension,
        ...
        self.embedding_module = get_embedding_module(
            ...
            edge_features=self.edge_raw_features, memory=self.memory,
            neighbor_finder=self.neighbor_finder, time_encoder=self.
            time_encoder, n_layers=self.n_layers,
            n_node_features=self.n_node_features,
            n_edge_features=self.n_edge_features,
            n_time_features=self.n_node_features,
            embedding_dimension=self.embedding_dimension, device=self.device,
    ...
    def compute_temporal_embeddings(
            self, source_nodes, destination_nodes, negative_nodes, edge_times,
        ...
        node_embedding = self.embedding_module.compute_embedding(
            memory=memory, source_nodes=nodes, timestamps=timestamps,
        ...
        if self.use_memory:
            if self.memory_update_at_start:
                ...
                assert torch.allclose(memory[positives], self.memory.get_memory(
                    positives), atol=1e-5), "Something wrong in how the memory was updated"
    ...
    def compute_edge_probabilities(
        ...
        score = self.affinity_score(torch.cat(
            ...
            torch.cat(
            [destination_node_embedding,
             negative_node_embedding])).squeeze(
            dim=0)

...
from tgn.utils.data_processing import compute_time_statistics, get_data
from tgn.utils.utils import (EarlyStopMonitor, RandEdgeSampler,
                             get_neighbor_finder)
...
parser.add_argument(
    '--node_dim', type=int, default=100,
    help='Dimensions of the node embedding')
parser.add_argument(
    '--time_dim', type=int, default=100,
...
parser.add_argument(
    '--message_function', type=str, default="identity",
    choices=["mlp", "identity"],
    help='Type of message function')
parser.add_argument('--memory_updater', type=str, default="gru", choices=[
    "gru", "rnn"], help='Type of memory updater')
parser.add_argument(
    '--aggregator', type=str, default="last",
...
parser.add_argument(
    '--memory_dim', type=int, default=172,
    help='Dimensions of the memory for '
    'each user')
parser.add_argument(
    '--different_new_nodes', action='store_true',
...
for i in range(args.n_runs):
    ...
    for epoch in range(NUM_EPOCH):
        ...
        if USE_MEMORY:
            tgn.memory.init_memory()

        # Train using only training graph
        tgn.set_neighbor_finder(full_ngh_finder)
        m_loss = []

        tgn.embedding_module.sample_time = 0
        tgn.memory_time = 0
        tgn.train()
        ...
        for k in range(0, num_batch, args.backprop_every):
            ...
            for j in range(args.backprop_every):
                ...
                sources_batch, destinations_batch = train_data.sources[
                    start_idx:end_idx], train_data.destinations[start_idx:end_idx]
                ...
                loss += criterion(pos_prob.squeeze(),
                                  pos_label) + criterion(neg_prob.squeeze(),
                                                         neg_label)
            ...
            if USE_MEMORY:
                ...
                tgn.memory_time += time.time() - start

        epoch_time = time.time() - start_epoch
        sample_time = tgn.embedding_module.sample_time
        memory_time = tgn.memory_time
        logger.info("epoch time {:.2f}s\tsample time: {:.2f}s\tmemory time: {:.2f}s".format(
                    epoch_time, sample_time, memory_time))
        ...
        val_ap, val_auc = eval_edge_prediction(
            model=tgn, negative_edge_sampler=val_rand_sampler, data=val_data,
            n_neighbors=NUM_NEIGHBORS)
        if USE_MEMORY:
    ...
    test_ap, test_auc = eval_edge_prediction(
        model=tgn, negative_edge_sampler=test_rand_sampler, data=test_data,
        n_neighbors=NUM_NEIGHBORS)
